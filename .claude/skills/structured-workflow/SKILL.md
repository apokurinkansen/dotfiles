...
**なぜリサーチが重要か:** Garbage in, garbage out。リサーチが間違えば計画も実装も間違う。実装が周辺システムを壊す最大の失敗モードは、構文エラーやロジックバグではなく、**既存のキャッシング層を無視する関数**、**ORM の慣例を無視するマイグレーション**、**既存ユーティリティと重複するロジックの再実装**といったシステム全体との不整合である。
...
**品質基準:** 表面的な概要ではなく、実装判断に必要な具体的詳細を含めること。research.md はユーザーが「Claude が本当に理解しているか」を検証するためのサーフェスでもある。誤解があればこの段階で修正できる。
...
**参考実装の活用:** ユーザーが良い実装例（オープンソースのコード、社内の既存実装など）を知っている場合、それを参考資料として共有してもらう。例: 「このライブラリの sortable ID の実装を参考に、同様のアプローチを採用する計画を書いて」。参考実装があると計画の品質が大幅に向上する。ユーザーに「参考にしたい既存実装はあるか」と積極的に尋ねること。
...
3. **インターフェース保護**: 既存 API やシグネチャの変更を禁止する（例: 「この3関数のシグネチャは変えるな」）
4. **技術選択のオーバーライド**: 特定の技術・ライブラリ・手法を指定する（例: 「このモデルを使え」「このライブラリの組み込みメソッドを使え」）
5. **焦点の修正**: 実装の方向がずれた時にフォーカスを戻す（例: 「今はエラーハンドリングではなくコアロジックに集中して」）
...
計画承認後、以下のような指示で実装を開始する:

> すべて実装して。タスク/フェーズが完了したら plan.md のチェックボックスをマークして。すべてのタスクが完了するまで止まらないで。不要なコメントや jsdoc は追加しない。型チェックを継続的に実行して。

この定型指示には以下の意図が込められている:
- 「すべて実装して」= 計画の全項目を実行する（つまみ食いしない）
...
3. 実装中の方針:
   ...
   - any や unknown 型を使わない（TypeScript の場合）

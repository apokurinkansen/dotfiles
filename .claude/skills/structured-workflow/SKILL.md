...
**なぜリサーチが重要か:** Garbage in, garbage out。リサーチが間違えば計画も実装も間違う。実装が周辺システムを壊す最大の失敗モードは、構文エラーやロジックバグではなく、**既存のキャッシング層を無視する関数**や**ORM の慣例を無視するマイグレーション**といったシステム全体との不整合である。
...
アノテーションサイクルでは、ユーザーは以下の 4 種類の介入ができる。すべてに従うこと:
